...
and admin_instr' =
  ...
  | Throwing of Tag.tagaddr * value stack
  | Rethrowing of int32 * (admin_instr -> admin_instr)
  | Label of int32 * instr list * code
  | Frame of int32 * frame * code
  | Catch of int32 * (Tag.tagaddr * instr list) list * instr list option * code
  | Caught of int32 * Tag.tagaddr * value stack * code
  | Delegate of int32 * code
  | Delegating of int32 * Tag.tagaddr * value stack
  ...
  let vs', es' =
    ...
    | Plain e', vs ->
      ...
      | TryCatch (bt, es', cts, ca), vs ->
        ...
        let n2 = Lib.List32.length ts2 in
        let args, vs' = take n1 vs e.at, drop n1 vs e.at in
        let cts' = List.map (fun (x, es'') -> (Tag.addr_of (tag frame.inst x), es'')) cts in
    ...
    | Label (n, es0, (vs', {it = Delegating (l, a, vs0); at} :: es')), vs ->
      vs, [Delegating (Int32.sub l 1l, a, vs0) @@ at]
    ...
    | Catch (n, cts, ca, (vs', {it = Throwing (a, vs0); at} :: es')), vs ->
      let ct = List.find_opt (fun (a', es'') -> a = a') cts in
      (match ct with
      | None ->
        (match ca with
         | None ->
    ...
    | Delegate (l, (vs', {it = Throwing (a, vs0); at} :: es')), vs ->
      vs, [Delegating (Int32.succ l, a, vs0) @@ e.at]
