...
let rec check_instr (c : context) (e : instr) (s : infer_stack_type) : op_type =
  ...
  | TryCatch (bt, es, cts, ca) ->
    ...
    check_catch_all ca c_catch ft e.at;
    ts1 -->... ts2
...
and check_catch (ct : var * instr list) (c : context) (ft : func_type) at =
  ...
  let ctt = FuncType (ts1, ts2) in
  check_block c es ctt at

and check_catch_all (ca : instr list option) (c : context) (ft : func_type) at =
  match ca with
  | None -> ()
  | Some es ->
    let FuncType (_, ts2) = ft in
    let cat = FuncType ([], ts2) in
    check_block c es cat at
